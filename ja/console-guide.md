...
| 韓国(光州) | https://static.toastoven.net/toastcloud/sdk_download/backup/kr3/scripts/linux/bootstrap.sh |
...
Windows用エージェントは、[NHN Cloudのダウンロードページ](/Download/)からダウンロードしてインストールします。インストール中、 MC ServerとMC Domain情報を入力する必要があります。MC Domainはユーザーのドメイン情報です。MC Serverには次の情報を入力します。

| リージョン | MC Server |
| --- | --- |
| 韓国(パンギョ) | tcbackup1.toastmaker.net |
| 韓国(ピョンチョン) | kr2-backup-mc1.cloud.toast.com |
| 韓国(光州) | tcbackup3.toastmaker.net |
...
バックアップ結果のバックアップパスを選択したり、一部のパスを選択できます。

<br/>

* **復元対象サーバー**

バックアップエージェントがインストールされているサーバーの中から選択できます。
復元元のサーバーと同じOS系列(LinuxまたはWindows)のサーバーを選択する必要があります。

<br/>

* **保存パス**

保存先のパスは任意のパスを指定でき、入力したパスが存在しない場合は、パスが自動的に作成されて復元が実行されます。
入力したパスに、復元データが上書き保存されます。
